...
| R04 | T04 - Information Disclosure | @mariasanchez0 | Consulta ao prontuário, ao histórico clínico, às prescrições ativas, aos dados de convênio e aos valores de consulta/exame de múltiplos pacientes por um perfil autenticado sem vínculo declarado com o atendimento, obtida ao variar sequencialmente o identificador em `buscarPacientePorIdentificador(idPaciente)`, configurando coleta em massa de dados clínicos e financeiros sensíveis | Farmácia/Medicamentos (RF21-RF24) e Financeiro/Cobrança (RF25) não têm microsserviço próprio e residem implicitamente no Serviço de Paciente e no Serviço de Atendimento Médico, multiplicando o alcance de qualquer acesso indevido; `idPaciente` é sequencial e a operação não verifica vínculo entre quem consulta e o paciente consultado; apenas o Administrador possui `nivelAcesso` graduado, de modo que Médico, Enfermeiro e Recepcionista têm o mesmo alcance de leitura entre si; o SGBD é único e sem segregação visível de dado sensível, contrariando o RNF05; e o Tópico 9 exclui do escopo o registro de eventos críticos de acesso indevido, de modo que uma sequência de consultas atípicas não gera alerta | 4 | 4 | 16 | Crítico |
...
**Probabilidade (4 - Alta).** O único obstáculo entre quem consulta e o dado é estar autenticado - não importa com qual dos quatro perfis, porque a seção 8.3 já registra que apenas o Administrador possui `nivelAcesso` graduado, então Médico, Enfermeiro e Recepcionista têm, perante o sistema, o mesmo alcance de leitura entre si. A partir daí, `Paciente.buscarPacientePorIdentificador(idPaciente)` recebe um identificador sequencial sem qualquer verificação de vínculo entre quem pergunta e o paciente perguntado: variar o número é suficiente para percorrer toda a base, sem exigir nenhuma condição adicional, nenhuma falha a explorar além da ausência de checagem que já é o estado normal do sistema. É a mesma estrutura de argumento que sustenta o valor 4 em R01 - "condições previsíveis do sistema" -, só que aqui o vetor não depende sequer de descobrir uma senha: quem já está dentro está pronto para consultar. Duas condições do próprio SIGH reforçam o valor sem depender de suposição: o RNF05 exige criptografia para as informações médicas e o modelo de dados não mostra segregação correspondente; e o Tópico 9 exclui do escopo o registro de eventos críticos de acesso indevido, de modo que uma sequência de consultas atípicas - o padrão que trairia uma coleta em massa - não encontra observador nem barreira.

**Impacto (4 - Muito alto).** O que está exposto não é um campo isolado: Farmácia/Medicamentos (RF21-RF24) e Financeiro/Cobrança (RF25) não têm microsserviço próprio e residem implicitamente dentro do Serviço de Paciente e do Serviço de Atendimento Médico, de modo que um único acesso indevido a qualquer um dos dois entrega, no mesmo movimento, prontuário, histórico, alergias, prescrições ativas, convênio e valores cobrados - não existe uma segunda barreira entre "ver quem é o paciente" e "ver o que ele toma, o que tem e quanto custa". Dados de saúde são dados pessoais sensíveis pela LGPD (art. 5º, II), com tratamento condicionado a hipóteses específicas (art. 11) e protegidos por sigilo médico; alergias, comorbidades e medicações em uso revelam condições estigmatizantes cuja exposição não se repara com correção de registro, ao contrário de um dado cadastral incorreto. E porque o identificador é enumerável, um incidente aqui não é de um paciente: é da base inteira, o que aciona o dever de comunicação à ANPD e aos titulares (art. 48). É exatamente o critério de "pode afetar muitos usuários... ou causar prejuízo grave" da escala 13.2.

**Quem é afetado.** Todos os pacientes cadastrados, na privacidade e na dignidade - alergias e comorbidades expostas a colegas de trabalho, familiares ou terceiros não têm como voltar a ser privadas; os pacientes com atendimento em Farmácia ou Financeiro, cujos dados adicionais (medicação em uso, valores cobrados, convênio) vazam pelo mesmo movimento que expõe um dado cadastral simples; e o hospital, exposto perante a LGPD com o agravante de que o próprio RNF05 já previa a proteção que faltou, e sem meio de demonstrar, a posteriori, quais registros foram efetivamente lidos - o Tópico 9 não deixou rastro para a apuração.

**Por que Crítico é adequado.** 4 × 4 = 16 empata com R01 no topo da escala, e a coincidência não é acidental: os dois riscos compartilham a mesma causa-raiz, a ausência de qualquer segunda barreira depois da autenticação. A diferença entre eles está na natureza do dano, não na gravidade - R01 é habilitador de ação (assumir a identidade para agir), R04 é habilitador de exposição (ler sem agir), e a seção 8.5.1 da Etapa 1 já registra que os dois se somam: uma conta assumida por R01 alcança exatamente o mesmo prontuário que R04 descreve, o que faz de R04 tanto uma ameaça autônoma - basta estar autenticado com qualquer perfil legítimo - quanto uma consequência agravada de R01 e de R06 (que expõe as credenciais de todos os perfis de uma vez). Tratar R04 fecha o vazamento pela via mais barata - variar um número -, mas não fecha o vazamento pela via da conta assumida; por isso seu tratamento precisa ser lido junto ao de R01 e R06, não isoladamente.

## 13.6 Priorização geral

> Seção compilada por **@mariasanchez0**, com validação coletiva - ordem de tratamento considerando pontuação, gravidade das consequências, usuários afetados, importância do ativo, possibilidade de recuperação, dependências e urgência.

Com os seis riscos registrados, a ordem de tratamento não segue direto a pontuação de 13.4, pelo mesmo motivo que 13.3 já antecipa: "dois riscos com a mesma pontuação podem receber prioridades diferentes" - e aqui o inverso também vale, um risco de pontuação menor pode exigir tratamento anterior a um de pontuação maior, quando ele é a causa de que os outros dependem.

**Ordem de tratamento:**

1. **R01 (16, Crítico)** - maior pontuação da escala, e habilitador central: os casos de abuso CA02, CA03 e CA04 citam nominalmente a conta assumida como caminho de entrada alternativo para adulterar prescrição, registrar óbito sem autoria e coletar dado indevidamente. Tratar R01 reduz a probabilidade efetiva de R02, R03, R04 e R06 antes mesmo de os controles próprios deles existirem.
2. **R06 (8, Alto)** - a pontuação é a menor do registro, mas 13.5 já argumenta, na justificativa do próprio @PPrauchner, que R06 "pede urgência acima do que um 8 sugere": é o único risco que multiplica os outros, porque expõe `nomeLogin` e `senhaLogin` de todos os perfis de uma vez, habilitando T01 em massa sem que nenhuma senha precise ser roubada individualmente. Sobe para a segunda posição porque, sem tratá-lo, qualquer avanço em R01 continua vulnerável a ser contornado em atacado - um Diretor indevido não precisa adivinhar senha alguma, ele já as tem todas.
3. **R04 (16, Crítico)** - empata com R01 na pontuação máxima, mas ocupa a posição imediatamente após os dois habilitadores porque, diferente deles, R04 já é o dano final, não um caminho para outro risco: a exposição em massa de prontuário e dado financeiro é o evento que R01 e R06 apenas facilitam chegar mais rápido. É o risco de maior alcance por incidente - "a base inteira, não um paciente" - e o único, junto de R01, que aciona diretamente o dever de comunicação à ANPD.
4. **R02 (12, Crítico)** - abaixo de R04 na ordem apesar da mesma classificação de nível, porque seu alcance por evento é menor (um paciente por alteração, contra a base inteira de R04), mas acima de R03 e R05 porque é o único risco cujo dano é físico e potencialmente fatal, e a janela de correção se fecha no instante da administração do medicamento - não existe tratamento posterior possível, como 14.6 já registra ao manter o impacto em 4 mesmo no residual.
5. **R03 (12, Crítico)** - o dano não é físico nem alcança muitos pacientes de uma vez, mas R03 tem uma propriedade que nenhum outro risco deste registro tem: ele compromete a **capacidade de provar** qualquer um dos demais. Sem autoria confiável, nem R01 nem R02 nem R04 podem ser apurados com certeza depois de ocorridos - é um habilitador de impunidade, não de acesso, e por isso fica à frente de R05.
6. **R05 (12, Crítico)** - mesma pontuação de R02 e R03, mas na última posição entre os Críticos porque é o único risco do recorte com **recuperação real e razoavelmente rápida**: a indisponibilidade se resolve restaurando o serviço, o que nenhum dos outros cinco riscos admite para o dano já consumado. É também o único que não exige ator malicioso, o que o torna mais previsível de antecipar por capacidade de engenharia (cota, réplica, modo degradado) do que por controle de acesso - motivo adicional para tratá-lo depois dos riscos de identidade e de exposição, que competem pelos mesmos controles de base (autenticação e autorização).

**Critérios que sustentaram a ordem, além da pontuação:**

- **Dependência/encadeamento** moveu R06 (8) para a segunda posição, à frente de quatro riscos de pontuação maior, e é o desvio mais expressivo desta priorização em relação à pontuação bruta.
- **Alcance do incidente** separou R04 de R02, R03 e R05 mesmo com pontuações próximas ou iguais: um evento de R04 atinge potencialmente toda a base de pacientes, os outros atingem um caso por vez.
- **Irreversibilidade do dano** ordenou R02 à frente de R03 e R05: o dano físico não admite qualquer forma de correção, nem mesmo a que R03 obtém por retificação junto ao Sistema Governamental.
- **Papel habilitador sobre a apuração** (não sobre o acesso) colocou R03 à frente de R05: sem ele, o próprio sucesso do tratamento dos outros riscos não pode ser demonstrado quando questionado.
- **Possibilidade de recuperação** foi o critério que manteve R05 na última posição entre os Críticos, apesar da pontuação igual a R02 e R03: é o único risco cujo dano se desfaz restaurando o serviço.

Esta ordem é a base da compilação de 14.5, e deve ser revista se os controles de R01 ou R06 não avançarem no prazo esperado, porque as duas primeiras posições sustentam a redução de probabilidade de quase todos os demais riscos do registro.
...
| R04 | Reduzir | Consultar prontuário e dados de convênio é função essencial do SIGH - Evitar eliminaria o próprio atendimento. Compartilhar não se aplica: a sensibilidade do dado clínico e financeiro é do hospital, e nenhum terceiro assume a responsabilidade de decidir quem pode lê-lo. Aceitar é insustentável em um risco que empata com o de maior pontuação da escala e cujo incidente, por ser enumerável, atinge a base inteira de uma vez. Resta Reduzir, atuando nas duas frentes que a análise de 13.5 separa: fechar o vetor barato (identificador enumerável, ausência de vínculo) e reduzir o raio de impacto quando um acesso indevido ainda assim ocorrer (isolamento entre módulos, minimização de dados retornados) |
...
- **Govern:** não existe, hoje, uma política de mínimo privilégio ou de necessidade de conhecer (*need-to-know*) que diga quem pode consultar o prontuário de qual paciente - o único critério de acesso é estar autenticado, com qualquer um dos quatro perfis. É a mesma lacuna estrutural de R06 (falta dono para decidir quem pode o quê), aplicada à leitura em vez de à escrita: sem essa decisão, "exigir vínculo profissional-paciente" não tem parâmetro para ser configurado - vínculo por internação? por especialidade? por plantão?
- **Protect:** é onde atuam os controles do plano (14.4) - identificador não enumerável, verificação de vínculo no servidor, isolamento entre Farmácia/Financeiro e o restante, minimização do dado retornado. Reduzem diretamente a probabilidade (fechando o vetor barato) e o impacto (reduzindo o raio de um acesso que ainda assim ocorrer).
- **Detect:** hoje uma sequência de consultas a pacientes distintos, sem relação entre si, passa despercebida - o Tópico 9 exclui esse registro do escopo. A detecção de padrão de consulta atípico é o que permite perceber a coleta em massa **durante** o evento, não apenas depois que o dado já apareceu fora do hospital, que é como a Etapa 1 descreve que a instituição hoje tende a descobrir o vazamento.
- **Respond:** havendo suspeita de coleta indevida, é preciso procedimento definido - identificar quais registros foram efetivamente acessados a partir da trilha de auditoria (DA01), avaliar o alcance do incidente e, dado o caráter enumerável do vetor, tratar como potencialmente exposta toda a faixa de identificadores percorrida, não apenas o último registro consultado.
- **Recover:** marcado, mas com o mesmo limite que separa R04 de R02 e R06: o **fato de o dado ter sido lido não se desfaz**. O que se recupera é a postura de confidencialidade daqui em diante - revogar ou revisar o acesso, fechar o vetor que permitiu a leitura - e, sobretudo, cumprir a obrigação regulatória: apurar o alcance do incidente pela trilha de auditoria e notificar a ANPD e os titulares afetados quando a apuração confirmar exposição (art. 48 da LGPD). É recuperação de processo e de conformidade, não do dado em si.
- **Por que não Identify:** o ativo (`Paciente`, `AtendimentoPaciente`, os campos de Farmácia e Financeiro sem microsserviço próprio), o vetor (`buscarPacientePorIdentificador`) e a lacuna (ausência de vínculo e de segunda barreira) já foram levantados na Etapa 1 (T04/CA04). Marcar Identify aqui seria preencher a tabela sem resultado esperado novo - a mesma razão que R01, R02, R03 e R06 já registraram.
...
| R04 | Reduzir | **R04-C1** - isolar Farmácia/Medicamentos (RF21-RF24) e Financeiro/Cobrança (RF25) do restante do Serviço de Paciente e do Serviço de Atendimento Médico, com fronteira de autorização própria, para que um acesso indevido a dado cadastral não alcance no mesmo movimento medicação em uso e valores cobrados; **R04-C2** - toda consulta a prontuário passa a exigir vínculo verificado no servidor entre o profissional autenticado e o paciente consultado (atendimento em curso, internação ativa ou plantão vigente), decidido pelo mesmo mecanismo de autorização de que depende o R06-C1; **R04-C3** - substituir o identificador sequencial de `buscarPacientePorIdentificador(idPaciente)` por um identificador não enumerável (opaco ou aleatório), com paginação e limite de taxa nas consultas de busca; **R04-C4** - minimizar o dado retornado por operação, devolvendo apenas os campos que a tela solicitante efetivamente usa, e criptografar em repouso os campos sensíveis do prontuário, da prescrição e do valor cobrado, atendendo ao RNF05; **R04-C5** - alerta de padrão de consulta atípico (volume de pacientes distintos consultados pela mesma sessão em janela curta, ou consulta fora do vínculo declarado), alimentando a trilha de auditoria (DA01) | Govern, Protect, Detect, Respond, Recover | @mariasanchez0 | Teste que varia o identificador sem vínculo declarado e comprova recusa; teste que confirma a ausência de dado de Farmácia/Financeiro na resposta de uma consulta cadastral simples; consulta à trilha de auditoria exibindo autor, sessão e paciente consultado; alerta de padrão de consulta disparando em cenário simulado de varredura sequencial; amostra da resposta de API mostrando apenas os campos mínimos e os valores sensíveis cifrados em repouso |
...
> Seção compilada pelo **@mariasanchez0**, com justificativa - depende dos controles de todos os riscos.

A ordem segue três critérios, na mesma linha do que orienta a priorização de 13.6: controles sem dependência interna e que reduzem mais de um risco vêm primeiro; controles que dependem de uma decisão de arquitetura (DA01/DA02/DA03, Etapa 3) vêm depois da própria decisão; e controles de segunda camada - confirmação humana, segunda assinatura, MFA - vêm por último, porque só têm valor pleno depois que a identidade e a autorização de base já estiverem sólidas.

| Onda | Controles | Risco(s) | Por que nesta posição |
| --- | --- | --- | --- |
| 0 | DA03 (serviço de autenticação) e DA01 (serviço de auditoria) | Transversal | Decisões de arquitetura da Etapa 3, não controles de 14.4, mas pré-requisito de tudo que segue: sem identidade verificada centralizada (DA03) e sem onde gravar a trilha (DA01), nenhum controle de auditoria ou de sessão tem onde operar |
| 1 | R01-C1 (hash e salt) | R01 | Não depende de nada, é pré-requisito dos demais controles de R01 e reduz o impacto residual de R06 enquanto a senha estiver em texto simples (14.6) |
| 1 | R06-C1 (revalidação de autorização no servidor) | R06 | Não depende de nada além de DA03/DA02; é o mesmo controle de que R02-C2 e R04-C2 dependem - a única implementação que reduz três riscos ao mesmo tempo |
| 1 | R04-C3 (identificador não enumerável) | R04 | Controle isolado, sem dependência; fecha o vetor mais barato de R04 (variar o número) antes mesmo de a checagem de vínculo existir |
| 2 | R02-C2 (papel e vínculo validados no servidor); R04-C2 (vínculo profissional-paciente) | R02, R04 | Dependem do mecanismo que R06-C1 institui |
| 2 | R01-C3, R01-C4, R01-C5 (bloqueio, expiração de sessão, política de senha) | R01 | Complementam R01-C1 sobre a mesma base de autenticação |
| 2 | R06-C2 (nivelAcesso imutável pelo titular) | R06 | Complementa R06-C1 |
| 3 | R02-C4 (versionamento imutável); R03-C1 a R03-C3 (autoria, carimbo do servidor, trilha append-only); R06-C3 (trilha de alteração de perfil); R04-C5 (alerta de padrão de consulta) | R02, R03, R06, R04 | Todos dependem do Serviço de Auditoria (DA01) existir; podem ser implementados em paralelo entre si, uma vez que a onda 0 esteja pronta |
| 4 | R04-C1 (isolamento Farmácia/Financeiro) | R04 | É o controle mais custoso de R04, porque mexe em fronteira de serviço; reduz o raio de impacto de qualquer vazamento futuro, inclusive dos outros riscos de leitura indevida, mas não é urgente sozinho - o vínculo (onda 2) e o identificador não enumerável (onda 1) já fecham a maior parte da probabilidade antes que essa fronteira exista |
| 4 | R05-C1 a R05-C4 (cota de conexões, rate limiting, réplica de leitura, timeout do Convênio) | R05 | Dependem do levantamento de capacidade (função Identify, única marcada nesta trilha) |
| 5 | R01-C2 (MFA e reautenticação); R02-C5 (segunda assinatura); R03-C5 (confirmação por segunda identidade); R06-C4 (alerta ao Diretor) | R01, R02, R03, R06 | Controles de segunda camada - só têm valor pleno depois que a identidade (onda 0-1) e a autorização (onda 1-2) de base já existirem; exigem coordenação humana, que custa mais para operacionalizar do que para especificar |
| 5 | R04-C4 (minimização de dado retornado e criptografia em repouso) | R04 | Pode evoluir em paralelo às ondas anteriores, mas depende de o vínculo (R04-C2) já existir para não quebrar telas que hoje dependem do registro completo |
| 6 | R05-C5 (alerta de 80% do pool) | R05 | Depende do levantamento de capacidade da onda 4 estar concluído e calibrado |

A regra geral por trás das ondas: nenhum controle de detecção (Detect) entra antes do controle de Protect equivalente estar de pé - alertar sobre um padrão que o sistema ainda nem consegue impedir produz ruído, não segurança. E nenhum controle de segunda camada (MFA, segunda assinatura, confirmação) entra antes da autorização de base, porque a segunda camada existe para elevar a barra de algo que já está minimamente protegido - reforçar uma porta que ainda não tem fechadura não muda o risco.
...
| R04 | Crítico (16) | Médio (6) | Controles R04-C1 a R04-C5 comprovadamente operantes (evidências de 14.4), com o identificador não enumerável em produção e o isolamento Farmácia/Financeiro implementado; alerta de padrão de consulta ativo e monitorado; revisão trimestral de amostras da trilha de acesso a prontuário; revisão da estimativa se o tratamento de R01 e R06 não avançar, porque uma conta assumida ou um perfil elevado contornam o vínculo profissional-paciente por dentro da própria sessão legítima |
...
### Justificativa do residual de R04 (@mariasanchez0)

**Probabilidade (4 → 2).** Os dois vetores que hoje não exigem nenhuma condição especial deixam de existir por construção: com o identificador não enumerável (R04-C3), variar um número deixa de bastar para percorrer a base; com o vínculo profissional-paciente validado no servidor (R04-C2), estar autenticado deixa de ser suficiente - é preciso também estar vinculado ao atendimento, à internação ou ao plantão do paciente consultado. O que resta é o evento que depende de condição específica: um profissional legitimamente vinculado que consulta além do que o atendimento exige - por curiosidade, por uma internação anterior do mesmo paciente, por parentesco -, o que a escala 13.1 descreve como "possível, mas dependente de uma vulnerabilidade ou condição específica". Não desce a 1 porque o vínculo, uma vez concedido, não distingue consulta necessária de consulta indevida dentro do mesmo atendimento - falta ao modelo uma segunda camada de minimização por campo (por que a Recepção precisa ver alergia?), que R04-C4 reduz, mas não fecha sozinho.

**Impacto (4 → 3).** O isolamento entre Farmácia/Financeiro e o restante (R04-C1) muda o que um acesso indevido, ainda ocorrendo, consegue alcançar: deixa de ser prontuário completo, medicação e valor cobrado no mesmo movimento, e passa a exigir comprometer mais de uma fronteira para chegar ao mesmo alcance de antes. Some-se a minimização de dado retornado (R04-C4), que reduz o que cada consulta individual expõe mesmo quando autorizada. Sai da faixa "pode afetar muitos usuários... e causar prejuízo grave" para "causa prejuízo relevante aos usuários, à privacidade" da escala 13.2 - ainda alto o suficiente para não cair a 2, porque o dado, uma vez visto, continua sendo dado de saúde protegido por LGPD e sigilo médico, e a exposição continua sendo, por natureza, irreparável por correção de registro.

**Condição para aceitar o residual Médio:** os cinco controles precisam estar comprovadamente operantes (evidências de 14.4), com destaque para o identificador não enumerável e o isolamento de módulos - os dois únicos que mudam a estrutura do vetor, não apenas o observam. A estimativa deve ser refeita se o tratamento de R01 não avançar (uma conta assumida herda o vínculo do titular, contornando R04-C2 por dentro) ou se o de R06 não avançar (um perfil elevado indevidamente alcança o cadastro completo e, com ele, qualquer vínculo que precise simular) - a mesma dependência que os residuais de R03 e R06 já registraram em relação a R01, e que aqui se soma a uma segunda, própria de R04: nenhum controle desta trilha impede a consulta de quem **já tem** vínculo legítimo e o usa além do necessário.
