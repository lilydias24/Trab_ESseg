...
- [ ] Etapa 3 - RS01 (@lilydias24) revisado e fechado: numeração do OWASP Top 10:2025 corrigida (A07 - Authentication Failures, A04 - Cryptographic Failures). RS02 revisado sem achados. RS03 (@PPrauchner) revisado, com uma frase de esclarecimento ainda sugerida para a cláusula 6 (@mariasanchez0). Diagrama da arquitetura segura concluído e versionado (@lorenzoficher), com o arquivo-fonte editável em `diagrams/estrutura/Diagramas_SIGH - Arquitetura segura.mmd`. DA01 (@lorenzoficher), DA02 (@mariasanchez0) e DA03 (@lilydias24) concluídas, com o quadro dos cinco campos do §18.4 na abertura da seção 4. Os critérios RS03-CA02 e RS03-CA04, que se contradiziam, foram conciliados depois que a implementação da Etapa 4 expôs a contradição. Conciliação entre a DA02 e as demais decisões concluída (@mariasanchez0): a cota por microsserviço do R05-C1 cobre também o Serviço de Autorização, sem abrir exceção na negação por padrão de RS03
...
- [x] Etapa 5 - sessão conduzida em 13/08/2026 (@PPrauchner); análise dos 3 achados concluída (@mariasanchez0). Revisão cruzada do @lorenzoficher concluída (seção 5), com o complemento da tabela final e 4 achados - dois altos, pendentes de correção pela @mariasanchez0: o impacto declarado do V03 descreve efeito que um `Access-Control-Allow-Origin: *` não produz, e o V01 é ancorado no R02-C3, que trata de validação semântica e não de injeção. Revisão cruzada da @lilydias24 concluída (seção 6): reprodutibilidade das seções 1-2 conferida contra os artefatos versionados (plano de automação, relatórios ZAP, capturas), sem erro encontrado; RV02/RV03 confirmados de forma independente; um ponto de evidência sinalizado (RL01 - dois números da sessão sem log versionado que os sustente)
